...
try {
  ...
  const runItem = (id: string, runId: string, elapsedMs: number, status: string, createdAt: string) => ({
    PK: `COMP#${id}`, SK: `RUN#${runId}`, runId, laneId: "fixture", startDeviceTs: 0,
    stopDeviceTs: elapsedMs, elapsedMs, splits: [], minTimeMs: 1000, maxTimeMs: 5000, status, createdAt });
  ...
  if (category.ranked.map((item: { teamName: string }) => item.teamName).join(",") !== "Gamma,Beta,Alpha") throw new Error(`Unexpected ranking: ${JSON.stringify(category.ranked)}`);
  ...
  console.log("PASS timeout correction penalty dq ranking privacy freeze-export reopen-cleanup");
