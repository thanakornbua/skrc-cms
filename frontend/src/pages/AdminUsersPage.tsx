  ...
  return <div className="page page-wide user-management-page">
    ...
    {showCreate && <form className="card user-create-card" onSubmit={handleCreate}>
      ...
      <div className="form-grid">
        ...
        <div className="field"><label htmlFor="new-user-competitor-id">{t("หมายเลขผู้เข้าแข่งขัน (ถ้ามี)", "Competitor ID (optional)")}</label><input id="new-user-competitor-id" value={createDraft.competitorId} onChange={(event) => setCreateDraft((current) => ({ ...current, competitorId: event.target.value }))} /></div>
    ...
    <div className="user-manager-layout">
      ...
      <main>{!draft ? <div className="card empty-state">{t("เลือกบัญชีเพื่อดูและแก้ไข", "Select an account to review and edit")}</div> : <form className="card user-editor" onSubmit={handleSave}>
        ...
        <div className="form-grid">
          ...
          <div className="field"><label htmlFor="edit-user-competitor-id">{t("หมายเลขผู้เข้าแข่งขัน (ถ้ามี)", "Competitor ID (optional)")}</label><input id="edit-user-competitor-id" value={draft.competitorId ?? ""} onChange={(event) => setDraft((current) => current ? { ...current, competitorId: event.target.value || null } : current)} /></div>
        </div>
