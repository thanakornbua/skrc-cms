...
| D13 | Registration email mode | ✅ DECIDED | **Auto-confirm sign-up** (no verification email at sign-up). Cognito still sends forgot-password codes. A separate asynchronous SES worker sends bilingual registration-received and approval notifications from `registration@notify.suankularb.space`; business actions remain successful while delivery retries. |
