...
|---|---|---|---|
| Registration | `REG#<cognitoSub>` | `PROFILE` | `teamName`, `category`, `school`, `certificateLanguage` (`THAI`\|`ENGLISH`\|`BILINGUAL`), `advisorNameThai`, `advisorNameEnglish`, `advisorEmail`, `advisorPhone`, `student1NameThai`, `student1NameEnglish` (student 1 is team leader/correspondent), equivalent Thai/English fields and an explicit `student1FoodAllergy`/`student2FoodAllergy`/`student3FoodAllergy` declaration (`NONE` or details) for students 2–3, `contactEmail`, `contactPhone`, `pdpaConsent` (`accepted`, `version`, `at`, `retentionMonths`, `deleteBy`, `authorityConfirmed`, `language`), status/review fields, and `createdAt`. Registration is free; no payment/slip attributes. |
