...
| Competitor | `COMP#<competitorId>` | `PROFILE` | Created at approval by copying the template’s team, school, certificate, advisor, student, contact, PDPA consent, category, and `cognitoSub` fields; plus flow status, `checkedInAt`, `checkedInBy`, inspection timestamp, disqualification, and `createdAt`. Staff attribution is admin-only and is never returned to competitors or public views. |
