...
|---|---|---|---|
| Registration | `REG#<cognitoSub>` | `PROFILE` | `teamName`, `category`, optional `school`, `certificateLanguage` (`THAI`\|`ENGLISH`\|`BILINGUAL`), optional complete advisor group (`advisorNameThai`, `advisorNameEnglish`, `advisorEmail`, `advisorPhone`), `student1NameThai`, `student1NameEnglish` (required team leader/correspondent), equivalent Thai/English fields for optional students 2–3, and an explicit `student1FoodAllergy` declaration plus declarations for each optional listed student (`NONE` or details), `contactEmail`, `contactPhone`, `pdpaConsent` (`accepted`, `version`, `at`, `retentionMonths`, `deleteBy`, `authorityConfirmed`, `language`), status/review fields, and `createdAt`. Blank school/advisor fields support Rule 2.1 unaffiliated entrants. Blank Student 2/3 fields represent a 1- or 2-member team; teams contain 1–3 members per Rules 1.2(3) and 2.2. Registration is free; no payment/slip attributes. |
| Competitor | `COMP#<competitorId>` | `PROFILE` | Created at approval by copying the template’s team, school, certificate, advisor, student, contact, PDPA consent, category, and `cognitoSub` fields; plus flow status, `checkedInAt`, `checkedInBy`, inspection timestamp, disqualification, `createdAt`, and optional `updatedAt`. Staff attribution is admin-only and is never returned to competitors or public views. |
| Weight inspection | `COMP#<competitorId>` | `INSPECTION#CHECK_IN#<inspectionId>` or `INSPECTION#PRE_COMPETITION#<inspectionId>` | Immutable measurement with `stage`, positive `weightGrams`, inspector verdict (`PASS`\|`FAIL`), optional notes, `byUser`, and `at`. A passed pre-competition record advances the existing competitor inspection state only after a passed check-in record. |
| Device heartbeat | `DEVICE#<deviceId>` | `STATUS` | Latest laptop bridge status: lane, serial port, bridge session, state, optional detail, and `lastSeenAt`. Operational status only; not a timing source. |
...
| Category config | `CONFIG#CATEGORY#<cat>` | `PROFILE` | integer `minTimeMs`, `stageMaxTimeMs`, and a compatibility `stageMaxAttempts` map fixed to 3 for every internal stage, legacy `maxTimeMs` fallback, `updatedAt`, `updatedBy` |
| Penalty rule | `CONFIG#PENALTY#<ruleId>` | `PROFILE` | `ruleId`, `label`, integer `penaltyMs`, `active`, `updatedAt`, `updatedBy` |
| Competition state | `CONFIG#COMPETITION` | `STATE` | `phase` (`OPEN`\|`CONCLUDED`), internal `activeStage`, `eligibleCompetitorIds`, immutable-on-advance `stageResults`, one-time `brackets` per category (`drawnAt`, internal `drawnBy`, random positions, QF/SF/final/third-place matches, random first team, winner and completion time), final `results`, and conclusion/update audit fields |
...
| Competitor profile audit | `COMP#<competitorId>` | `AUDIT#PROFILE_UPDATE#<isoTs>` | `type`, `byUser`, `at`, mandatory `reason`, field-level `changes` (`before`/`after`), and PDPA `deleteBy`; written transactionally with the Competitor and source Registration profile updates |
