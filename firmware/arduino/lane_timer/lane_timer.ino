// Lane timer — Arduino IDE sketch.
//
// Behaviourally identical to firmware/src/main.cpp (the PlatformIO build).
// Two deliberate differences, both forced by the Arduino IDE:
//
//   1. config.h is included FIRST, because the Arduino IDE has no build flags,
//      so TRANSPORT_HTTP is defined in config.h rather than platformio.ini and
//      must be visible before the conditional includes below.
//   2. Internals are `static` instead of living in an anonymous namespace,
//      because the IDE's automatic prototype insertion is unreliable around
//      anonymous namespaces in a .ino file.
//
// Keep this in sync with firmware/src/main.cpp. gate_logic.h in this folder is
// a byte-identical copy of firmware/include/gate_logic.h; run
// firmware/arduino/check-sync.sh to verify it has not drifted.
//
// Requires: ESP32 board support, and ArduinoJson v7 (v6 will NOT compile —
...
static const int checkpointPins[] = CHECKPOINT_GATE_PINS;
static const char* checkpointIds[] = CHECKPOINT_GATE_IDS;
static Gate gates[2 + CHECKPOINT_GATE_COUNT];
static Event queue[QUEUE_CAPACITY];
static volatile size_t queueHead = 0;
static volatile size_t queueSize = 0;
...
static const char* typeName(GateType type) {
  switch (type) {
...
static bool enqueue(const Gate& gate, uint32_t timestamp) {
...
static void pollGates(uint32_t now) {
  for (Gate& gate : gates) {
...
static void discardHead(const char* eventId) {
  portENTER_CRITICAL(&queueMux);
...
static void scheduleRetry(const Event& attempted, uint32_t now, const char* reason) {
...
#if defined(TRANSPORT_HTTP)
static void drainQueue(uint32_t now) {
...
#else
static void emitSerialHead(uint32_t now) {
